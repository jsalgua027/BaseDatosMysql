...
-- EJER 2   
-- Comprueba que el departamento de las personas que ejercen la dirección de los 							departamentos pertenezcan a dicho departamento.
delimiter $$
...
-- EJER 3		
-- Añade lo que consideres oportuno para que las comprobaciones anteriores se 
-- hagan también cuando se modifiquen la fecha de nacimiento de un empleado o al director/a de un departamento.
																																							-- sobre ejer 1 CON UPDATE
delimiter $$
...
delimiter ;
								...
								-- sobre ejer 2 UPDATE DE LOS DEPARTAMENTOS
delimiter $$
...
-- EJER 4			
-- Añade una columna numempleados en la tabla departamentos. En ella vamos a almacenar el número de empleados de cada departamento.
alter table departamentos
	add column numempleados int not null default 0
    
-- EJER 5		
-- Prepara un procecdimiento que para cada departamento calcule el número de empleados y guarde dicho valor en la columna creada en el apartado 4.
delimiter $$
...
-- EJER 6	
-- Prepara lo que consideres necesario para que cada trimestre se compruebe y actualice, en caso de ser necesario, el número de empleados de cada departamento.
delimiter $$
...
-- EJER 7		
-- Asegúrate de que cuando eliminemos a un empleado, se actualice el número de empleados del departamento al que pertenece dicho empleado.
delimiter $$
...
-- EJER 8	
/*El profesorado también puede matricularse en nuestro centro pero no de las materias que imparte. Para ello tendrás que hacer lo sigjuiente:
...
alter table materias
	...
	add constraint fk_materias_profesorado foreign key (codprof) references profesorado(codprof);
																		...
																		-- SI LA FECHA DE PUBLICACION ES ANTERIOR A LA DE CREACION
...
Comprobar si tenemos suficiente stock para ello, si no es así, mostraremos un mensaje de no disponibilidad.				NO HAY STOCK
...
-- ejer 3
/*Queremos que, cuando queden menos de 5 unidades almacenadas  en nuestro almacén, se realice un pedido automático a nuestro proveedor.		HACER PEDIDO EN X UNIDAD
*/		
delimiter $$
...
-- 																											COMPROBA R QUE FECHAS NO SOLAPAN
delimiter $$
...
*/
																														-- ACTUALIZA EL STOCK AL HACER UNA VENTA
drop trigger if exists actualizastock;
delimiter $$
create trigger actualizastock
